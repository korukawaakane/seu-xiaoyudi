...
import type { SearchResult, SearchResultType } from "@/src/lib/content";
...
export function SearchExplorer({ items }: SearchExplorerProps) {
  ...
  const hasQuery = Boolean(normalizedQuery || normalizedTag);
  const results = hasQuery
    ? items.filter((item) => {
        ...
        const matchesTag =
          !normalizedTag ||
          item.tags.some((itemTag) => itemTag.toLocaleLowerCase() === normalizedTag);
  ...
  return (
    <>
      <form
        action="/search"
        className="grid gap-3 rounded-[8px] border border-line bg-white p-4 shadow-soft sm:grid-cols-[minmax(0,1fr)_auto]"
        role="search"
      >
      ...
      {tag ? (
        <div className="mt-5 flex flex-wrap items-center gap-3 text-sm text-muted">
          ...
          <Link
            className="action-link min-h-0 py-1"
                  ...
                  {result.tags.length ? (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {result.tags.map((item) => (
                        <Tag key={item} tone="bronze" value={item}>
                          {item}
                        </Tag>
                      ))}
                    </div>
